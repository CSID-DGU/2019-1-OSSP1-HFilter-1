...
def makeLabelByTxt_withID(path):
    ...
    for i in range(0, len(sortLabels)):
        ...
        for j in range(1, len(sortLabels[i][1])):
            output.write("  ID "+ str(j) + ": " + str(sortLabels[i][1][j]) + "\n")
...
# Error
if path_men == None or path_women == None or path_merge == None:
    sys.exit(1)
